...
QString NetworkCallable::replyGetSetCookie(QString key) {
    QString result;
    qDebug() << __func__ << "key: " + key;
    if (this->cookies.isValid()) {
        QList<QVariant> list = this->cookies.toList();
        for (const QVariant &var: std::as_const(list)) {
            QNetworkCookie cookie;
            if (var.canConvert<QNetworkCookie>()) {
                cookie = var.value<QNetworkCookie>();
...
QString NetworkCallable::replyGetHeader(const QString &headerName, const QString &defaultValue)
{
    for (auto &each : replyHeaders) {
        QString first(each.first);
        if (first.compare(headerName, Qt::CaseInsensitive) == 0) {
            return QString::fromUtf8(each.second);
        }
    }
    return QString();
}
...
FilDownloadParam::FilDownloadParam(QObject *parent) : QObject{parent} {
}

FilDownloadParam::FilDownloadParam(QString destPath, bool append, QObject *parent)
    : QObject{parent} {
...
NetworkParams *NetworkParams::addHeader(const QString &key, const QVariant &val) {
    qDebug() << __func__ << "key:" << key << "val:" << val;
...
NetworkParams *NetworkParams::addAttribute(int code, const QVariant &val)
{
    _attributesMap.insert(code, val);
    return this;
}
...
NetworkParams *NetworkParams::toDownload(QString destPath, bool append) {
    qDebug() << __func__ << "destPath:" << destPath << "append:" << append;
    _downloadParam = new FilDownloadParam(std::move(destPath), append, this);
    return this;
}
...
NetworkParams *NetworkParams::setDirectFormData(bool enable)
{
    _directFormData = enable;
    return this;
}

NetworkParams *NetworkParams::addHttpPart(QHttpPart part)
{
    _httpParts.append(part);
    return this;
}
...
void Network::handle(NetworkParams *params, NetworkCallable *c) {
    ...
    QThreadPool::globalInstance()->start([=]() {
        ...
        manager.setCookieJar(FilCookies::getInstance());
        // It must be separated from the parent object; otherwise, it will be destroyed.
        FilCookies::getInstance()->setParent(nullptr);
        manager.setTransferTimeout(params->getTimeout());
        if (params->_enableProxy) {
            manager.setProxy(params->_proxy);
        }
        QEventLoop loop;
        ...
        for (int i = 0; i <= params->getRetry() - 1; ++i) {
            ...
            qDebug() << __func__ << "Start Loop exec";
            loop.exec();
            if (conn_destroyed) {
            ...
            callable->replyHeaders = reply->rawHeaderPairs();
            ...
            if (httpStatus == 200) {
                if (!callable.isNull()) {
                    ...
                    callable->cookies = reply->header(QNetworkRequest::SetCookieHeader);
        ...
        connect(reply, &QNetworkReply::readyRead, reply,
                [reply, seek, destFile, cacheFile, callable] {
                    ...
                    qint64 contentLength =
                        reply->header(QNetworkRequest::ContentLengthHeader).toULongLong() + seek;
...
void Network::sendRequest(QNetworkAccessManager *manager, QNetworkRequest request,
    ...
    switch (params->_type) {
        case NetworkParams::TYPE_FORM: {
            ...
            } else if (params->_directFormData) {
                // The caller wishes to customize the form data. We will attempt to directly use the data provided by the form.
                auto *multiPart = new QHttpMultiPart();
                multiPart->setContentType(QHttpMultiPart::FormDataType);
                for (const auto &each : params->_paramMap.toStdMap()) {
                    QHttpPart part;
            ...
            } else if (params->_httpParts.size() != 0) {
                // If the caller has already prepared the data, we will directly use it.
                qDebug() << __func__ << "If the caller has already prepared the data, we will directly use it";
                auto *multiPart = new QHttpMultiPart();
                multiPart->setContentType(QHttpMultiPart::FormDataType);
                for (int i = 0; i < params->_httpParts.size(); ++i) {
                    multiPart->append(params->_httpParts.at(i));
                }
                reply = manager->sendCustomRequest(request, verb, multiPart);
                multiPart->setParent(reply);
                connect(reply, &QNetworkReply::uploadProgress, reply,
...
void Network::printRequestEndLog(const QNetworkRequest &request, NetworkParams *params,
    ...
    for (const QByteArray &header : std::as_const(headers)) {
        qDebug() << qUtf8Printable(
            QString::fromStdString("<Header> %1=%2").arg(header, reply->rawHeader(header)));
    }
...
void Network::addAttributes(QNetworkRequest *request, const QMap<int, QVariant> &attributes)
{
    QMapIterator<int, QVariant> iter(attributes);
    while (iter.hasNext()) {
        iter.next();
        request->setAttribute(static_cast<QNetworkRequest::Attribute>(iter.key()), iter.value());
    }
}
