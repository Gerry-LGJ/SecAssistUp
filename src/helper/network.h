...
public:
    ...
    // Extend Part
    QVariant cookies;
    Q_INVOKABLE QString replyGetSetCookie(QString key);
    QList<QNetworkReply::RawHeaderPair> replyHeaders;
    Q_INVOKABLE QString replyGetHeader(const QString &headerName, const QString &defaultValue = {});
};

/**
 * @brief The FilDownloadParam class
 */
class FilDownloadParam : public QObject {
    Q_OBJECT
public:
    explicit FilDownloadParam(QObject *parent = nullptr);

    FilDownloadParam(QString destPath, bool append, QObject *parent = nullptr);
...
public:
    ...
    Q_INVOKABLE NetworkParams *setDirectFormData(bool enable);

    Q_INVOKABLE NetworkParams *addHttpPart(QHttpPart part);
...
public:
    FilDownloadParam *_downloadParam = nullptr;
    ...
    QMap<int, QVariant> _attributesMap;
    int _timeout = -1;
    int _retry = -1;
    QVariant _openLog;
    ...
    bool _directFormData;
    QList<QHttpPart> _httpParts;
};
...
private:
    ...
    static void addAttributes(QNetworkRequest *request, const QMap<int, QVariant> &attributes);
